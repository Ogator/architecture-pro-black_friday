...
Как альтернатива можно инициализировать и наполнить кластер одним скриптом
...
Дождитесь, пока `healthy` станут `configSrv`, `shard1-1` и `shard2-1`.
...
Если выполнить шаг на ещё не готовом контейнере, `mongosh` ответит
`MongoNetworkError: connect ECONNREFUSED 127.0.0.1:27017`. Перед шагом 3 дайте
`mongos_router` несколько секунд после шага 2 — он поднимается уже после того, как
config-реплика получила PRIMARY.
